# enterprise-mcp-essentials

Customer-facing lab guides for Enterprise MCP Essentials, served via GitHub
Pages.
...
Pressed static output only (HTML from the Gutenberg press), under `docs/` — the GitHub Pages source folder (main branch, /docs). Source
manuscripts, decisions records, and screenshots-in-progress live in the
internal authoring repo. Do not edit lab HTML in place — re-press and replace the whole set. This
repo receives curated snapshots only: all iteration, discussion, and issue
tracking happen in the internal authoring repo, so nothing internal ever
accumulates in this repo's history or PR threads.
...
regime; a ruleset enforces this server-side. Before merging: rebase the
branch onto main, then run the scrubber (`scripts/publish-checks.sh`) on
the rebased result — CI repeats the same scrub on every push and PR.
